...
function thememount_body_classes($bodyClass){
	...
	$bodyClass[] = sanitize_html_class( 'thememount-header-style-'.trim($headerstyle) );
...
function howes_scripts_styles() {
	...
	wp_enqueue_script( 'howes-script', get_template_directory_uri() . '/js/functions.min.js', array( 'jquery', 'isotope' ), '2013-07-18', true );
...
function howes_register_required_plugins(){
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> get_stylesheet_directory() . '/inc/plugins/cf-post-formats.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
